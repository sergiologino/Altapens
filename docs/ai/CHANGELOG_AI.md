...
## 2026-04-17
- **Deploy / Docker backend:** `apps/backend/Dockerfile` — `# syntax=docker/dockerfile:1.4`, отдельный `RUN ./gradlew dependencies` до `COPY src`, кэш BuildKit для `~/.gradle`; снижает время и риск таймаута Coolify при `bootJar`; примечание в `RUN_LOCAL.md`
